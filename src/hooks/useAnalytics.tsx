...
export const useAnalytics = ({
        ...
        (weeksAgo = 0) => {
            ...
            weekDates.forEach((date) => {
                ...
                if (totals.calories > 0 || steps > 0) {
                    ...
                    if (Math.abs(totals.calories - targets.calories) <= 150)
                        calOkDays++;
                    // Protein goal check (>= 90%)
                    if (totals.protein >= targets.protein * 0.9) protOkDays++;
                    // Steps goal check (>= 8000 default)
                    if (steps >= stepGoal) stepsOkDays++;
                }
            });

            const avgCals =
                daysTracked > 0 ? Math.round(totalCals / daysTracked) : 0;
            const avgProt =
                daysTracked > 0 ? Math.round(totalProt / daysTracked) : 0;
            const avgSteps =
    ...
    const getWeeklyData = useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            days.push({
                name: new Date(date + 'T12:00:00').toLocaleDateString('es-AR', {
                    weekday: 'short',
                }),
