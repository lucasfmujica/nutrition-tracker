...
import { formatTime, getArgentinaDateString, getArgentinaTimestamp } from '../utils/dateUtils';
...
export const useQuickLog = (foodLog, saveFoodEntry) => {
  ...
  const { frequentFoods, frequentCombos } = useMemo(() => {
    ...
    const foodFrequency = {};
    const foodData = {}; // Store latest macro data for each food name

    // B. Combo Analysis Prep
    const sessionGroups = {}; // Key: "Date-Hour", Value: [foodItems]
    ...
    const topCombos = Object.entries(comboSignatures)
      ...
      .map(([signature]) => {
        ...
        return {
          ...
          totalCalories: names.reduce((sum, name) => sum + (foodData[name]?.calories || 0), 0)
        };
      });
  ...
  // 2. Quick Log Action
  const quickLog = useCallback(async (itemOrCombo) => {
