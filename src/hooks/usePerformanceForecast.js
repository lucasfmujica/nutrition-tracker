 ...
 *
 * Improved 'Prime' Algorithm with percentage-based trending for more variance:
 * - Compares 3-day vs 7-day moving averages for HRV/Readiness and Sleep
 * - Uses percentage thresholds instead of strict greater/less than
 * - Adds intermediate states for more meaningful daily changes
 *
  ...
  return useMemo(() => {
    ...
    const getAvgMetrics = (daysBack) => {
      ...
      for (let i = 0; i < daysBack; i++) {
        ...
        if (entry) {
          const readiness = entry.readiness_score || entry.readinessScore || 0;
          const sleep = entry.sleep_score || entry.sleepScore || 0;
          if (readiness > 0) {
      ...
      return count > 0 ? { readiness: readinessSum / count, sleep: sleepSum / count, count } : null;
    };
    ...
    // Calculate percentage change from 7-day to 3-day average
    const readinessTrendPct = ((avg3d.readiness - avg7d.readiness) / avg7d.readiness) * 100;
    ...
    const todayReadiness = todayEntry?.readiness_score || todayEntry?.readinessScore || avg3d.readiness;
    const todaySleep = todayEntry?.sleep_score || todayEntry?.sleepScore || avg3d.sleep;
    ...
    const volume48h = getDailyVolume(today) + getDailyVolume(addDaysToDate(today, -1));
    ...
    else if (readinessTrendPct >= 0 && todayReadiness >= 75 && todaySleep >= 70) {
      status = 'Good';
    ...
    // CAUTION: Declining trends but not critical
      ...
      status = 'Caution';
      title = 'Precaución';
      copy = '⚠️ Tendencia descendente detectada. Considera moderar la intensidad mañana.';
      icon = 'cloud';
    ...
    // RECOVERY: Both metrics clearly declining
      ...
      copy = '😴 Tu cuerpo necesita descanso. Prioriza sueño y movilidad mañana.';
      icon = 'cloud-rain';
    ...
    // REST PRIORITY: High recent volume with declining metrics
      ...
      copy = '💪 Alto volumen reciente. Dale tiempo a tus músculos para recuperarse.';
