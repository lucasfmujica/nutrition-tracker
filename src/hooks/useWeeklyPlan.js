...
export const useWeeklyPlan = () => {
   ...
   */
  const deleteDayPlan = useCallback(async (dayIndex) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
   ...
   * @param {Object|null} workoutData - { type, name, intensity } or null for rest day
   */
  const updateDayPlan = useCallback(async (dayIndex, workoutData) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('User not authenticated');
      ...
      setPlan(prev => ({
        ...prev,
        [dayIndex]: dataToSave.type === 'rest' ? null : dataToSave
      }));
