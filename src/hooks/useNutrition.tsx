...
export const useNutrition = (
    ...
    // Actions
    const saveFoodLog = async (newLog: FoodEntry[]) => {
        setFoodLog(newLog);
    };

    const saveFoodEntry = async (entry: FoodEntry): Promise<FoodEntry | null> => {
    ...
    const saveWaterLog = async (newLog: WaterEntry[]) => {
        setWaterLog(newLog);
        try {
    ...
    const saveWaterEntry = async (entry: WaterEntry) => {
        if (useCloud) {
            try {
    ...
    const addWaterGlass = async (date?: string): Promise<string | null> => {
    ...
    const removeWaterGlass = async (date?: string): Promise<string | null> => {
        const targetDate = date || getArgentinaDateString();
        const existingEntry = waterLog.find((e) => e.date === targetDate);
