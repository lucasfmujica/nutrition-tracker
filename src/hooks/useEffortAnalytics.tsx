 ...
 *
 * @param {Workout[]} workoutLog - List of workout entries
 * @param {OuraEntry[]} ouraLog - List of Oura readiness/sleep entries
 * @param {WeightAnalytics} weightAnalytics - Contains currentTrend (kg/week)
 * @param {string} [selectedDate] - Optional date to check (defaults to today in Argentina TZ)
 */
export const useEffortAnalytics = (
    workoutLog: Workout[],
    ouraLog: OuraEntry[],
    ...
    return useMemo(() => {
        ...
        const getDailyVolume = (date: string, log: Workout[]) => {
        ...
        const hasAnyWorkout = (date: string, log: Workout[]) => {
        ...
        for (let i = 0; i < 7; i++) {
            last7DaysVolume += getDailyVolume(
                addDaysToDate(targetDate, -i),
                workoutLog,
            );
        }
        ...
        for (let i = 0; i < 30; i++) {
            last30DaysVolume += getDailyVolume(
                addDaysToDate(targetDate, -i),
                workoutLog,
            );
        }
        ...
        const readiness = targetOura?.readinessScore || 75;
        const sleepScore = targetOura?.sleepScore || 75;
