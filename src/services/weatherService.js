...
export const getCurrentWeather = async () => {
  try {
    ...
    if (cachedData && cachedData.value) {
      ...
      const cache = JSON.parse(cachedData.value);
    ...
    if (!freshWeather) {
      ...
      if (cachedData && cachedData.value) {
        const cache = JSON.parse(cachedData.value);
        console.warn('[WeatherService] API failed, using stale cache as fallback');
        return cache;
      }

      console.error('[WeatherService] No weather data available (API failed, no cache)');
      return null;
    }
