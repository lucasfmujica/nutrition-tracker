...
export const ADJUSTMENT_CONSTANTS = {
    ...
    STEP_ADJUSTMENT_PCT: 0.1, // 10% shift
} as const;

interface RateOfLoss {
    min: number;
    max: number;
}
 ...
 */
export const calculateIdealRateOfLoss = (currentWeight: number): RateOfLoss => {
...
interface PerformanceAnalysis {
    status: 'slow' | 'fast' | 'onTrack' | 'gaining' | 'unknown';
    message: string;
 ...
 */
export const analyzeWeeklyPerformance = (
    ...
    if (currentTrend === null || currentWeight === null) {
        return {
            status: 'unknown',
            message: 'Faltan datos para analizar',
            severity: 'warning',
        };
    }
 ...
 */
export const proposeTargetAdjustments = (
    currentTargets: Partial<CustomTargets> & { steps?: number },
    currentWeight: number,
    currentTrend: number | null,
    profile: Profile,
): TargetProposal => {
    ...
    let reasoning = '';
    let actionType: 'reduce' | 'increase' | 'maintain' = 'maintain';
    ...
    switch (analysis.status) {
        ...
        case 'gaining':
            ...
            newCalories = Math.round(
                currentCalories * (1 - ADJUSTMENT_CONSTANTS.CALORIE_ADJUSTMENT_PCT),
            );
            reasoning =
                'El progreso se ha estancado. Reducimos un 5% las calorías para reactivar la quema de grasa.';
            actionType = 'reduce';
            break;
        ...
        case 'fast':
            ...
            reasoning =
                'Pérdida demasiado agresiva. Aumentamos calorias ligeramente para proteger tu masa muscular.';
            actionType = 'increase';
            break;
        ...
        default:
            reasoning =
                'Estás en el camino perfecto. Mantenemos los objetivos actuales.';
            actionType = 'maintain';
            break;
    }
...
export const formatWeeklySummary = (
    ...
    customTargets: Partial<CustomTargets> & { steps?: number },
    profile: Profile,
): WeeklySummary | null => {
    if (!weightAnalytics || weightAnalytics.currentTrend === null) return null;

    const { currentTrend, weeklyAdherence } = weightAnalytics;
    const { currentWeight } = profile;

    // Since we checked for null above, currentTrend is number here, but TS might need casting or check
    const trend = currentTrend as number;

    const proposal = proposeTargetAdjustments(
        customTargets,
        currentWeight,
        trend,
        profile,
    ...
    return {
        ...
        currentTargets: {
            calories: customTargets.calories || 0,
