import { CoachInsight, OuraEntry, WeightAnalytics } from '../types/domain';
 ...
 */
const analyzeSleepQuality = (ouraLog: OuraEntry[]): SleepQuality | null => {
...
const needsRecovery = (
    ...
    const highWorkoutLoad =
        workoutAnalysis.gymCount + workoutAnalysis.tennisCount >= 3;
 ...
 */
export const getWeeklyCoachInsight = (data: CoachInsightData): CoachInsight => {
    const {
        weightAnalytics = {
            currentTrend: null,
            weeklyAdherence: 0,
        ...
        ouraLog = [],
        workoutAnalysis = { gymCount: 0, tennisCount: 0 },
    ...
    if (weeklyAdherence === 100) {
        const adherencePraise =
            remainingWeight > 0 && estimatedGoalDate
    ...
    if (needsRecovery(ouraLog, workoutAnalysis)) {
        const sleepData = analyzeSleepQuality(ouraLog);
        // sleepData is not null here due to check in needsRecovery, but typescript might complain
        const entries = sleepData ? sleepData.entries : 0;
        const avgScore = sleepData ? sleepData.avgScore : 0;
    ...
    if (currentTrend !== null && currentTrend !== undefined) {
        ...
        if (currentTrend < -0.5) {
            const weeksToGoal =
                remainingWeight > 0
                    ? (remainingWeight / Math.abs(currentTrend)).toFixed(1)
                    : '0';
            return {
