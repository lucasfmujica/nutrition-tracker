...
export const OuraBentoGrid = ({ data }) => {
  if (!data) {
    return (
      <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 shadow-sm">
        ...
        <h3 className="text-gray-900 font-bold text-lg mb-1">Sin datos sincronizados</h3>
        <p className="text-gray-500 text-sm">Sincroniza tu anillo para ver el reporte.</p>
      </div>
    );
  }
  ...
  const {
    ...
    hrv, restingHr,
    sleepHours, deepSleepMins, remSleepMins,
    steps, active_calories // Note: active_calories might be undefined based on current mapping
  } = data;
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm tracking-wide text-indigo-100">READINESS</span>
            </div>
            {readinessScore >= 85 && (
          ...
          <div className="flex items-end gap-1 mb-8">
            <span className="text-6xl font-bold tracking-tighter">{readinessScore || '--'}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 rounded-xl p-3 backdrop-blur-sm border border-white/10">
               ...
               <span className="text-[10px] text-indigo-300 ml-1">ms</span>
            </div>
            <div className="bg-white/5 rounded-xl p-3 backdrop-blur-sm border border-white/10">
               <div className="flex items-center gap-2 mb-1">
               ...
               <span className="text-xl font-bold">{restingHr || '--'}</span>
               <span className="text-[10px] text-indigo-300 ml-1">bpm</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group hover:border-blue-200 transition-colors">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-gray-700">SUEÑO</span>
            </div>
            <span className={`text-xl font-bold ${getScoreColor(sleepScore)}`}>{sleepScore || '--'}</span>
          </div>

          <div className="flex items-baseline gap-1 mb-4">
            <span className="text-3xl font-bold text-gray-900">{sleepHours || '--'}</span>
            <span className="text-sm text-gray-500 font-medium">horas</span>
          </div>

          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="text-gray-500 font-medium flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div> Deep
              </span>
              <span className="font-bold text-gray-900">{formatMins(deepSleepMins)}</span>
            </div>
             <div className="flex justify-between items-center text-xs">
              <span className="text-gray-500 font-medium flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-sky-400"></div> REM
              </span>
              <span className="font-bold text-gray-900">{formatMins(remSleepMins)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group hover:border-orange-200 transition-colors">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-gray-700">ACTIVIDAD</span>
            </div>
            <span className={`text-xl font-bold ${getScoreColor(activityScore)}`}>{activityScore || '--'}</span>
          </div>
          ...
          <div className="space-y-4">
             <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-gray-900">{steps ? steps.toLocaleString() : '--'}</span>
             </div>
              {/* Optional: Add calories if available later */}
