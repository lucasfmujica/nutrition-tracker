 ...
 */
export const EffortRadar = ({ analytics, selectedDate }) => {
  ...
  // Check if Oura data is stale (from a different date than selected)
      ...
      {isFutureDate && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl flex gap-2 items-start">
          ...
          <p className="text-xs text-yellow-800 leading-snug">
            Fecha futura: Esta proyección se basa en tu historial. No hay datos reales de Oura o entrenamientos para esta fecha.
          </p>
        </div>
      )}

      {/* Stale Oura Data Warning */}
      {isOuraStale && !isFutureDate && (
        <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-xl flex gap-2 items-start">
          ...
          <p className="text-xs text-orange-800 leading-snug">
            <strong>Datos de Oura desactualizados:</strong> Los scores de readiness y sueño son del {metrics.ouraDate}. Sincroniza tu Oura Ring para obtener datos más recientes.
          </p>
        </div>
      )}

      {/* Metrics Breakdown (Expandable) */}
      {showMetrics && metrics && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl space-y-2">
          <p className="text-xs font-bold text-blue-900 mb-2">📊 Factores que influyen:</p>
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-1.5">
              <span className="text-xs">💍</span>
              <span className="text-xs text-blue-800">
                <strong>Readiness:</strong> {metrics.readiness}%
                {isOuraStale && <span className="text-orange-600"> ({metrics.ouraDate})</span>}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-xs">😴</span>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-blue-800">
                <strong>Volumen 7d/30d:</strong> {metrics.volumeRatio}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-xs">⚖️</span>
              <span className="text-xs text-blue-800">
                <strong>Tendencia:</strong> {metrics.trend ? `${metrics.trend.toFixed(2)} kg/sem` : 'N/A'}
              </span>
            </div>
          </div>
          <p className="text-[10px] text-blue-700 mt-2 italic">
            Readiness alto + volumen moderado = capacidad alta (zona izquierda/verde).
            Readiness bajo + volumen alto = necesitas recuperación (zona derecha/naranja).
          </p>
        </div>
      )}
