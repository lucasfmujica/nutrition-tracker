 ...
 *
 * Displays Intelligence Engine metrics:
 * - Estimated goal date (75kg target)
 * - Weight loss journey progress bar
 * - Current trend (kg/week)
 * - Weekly adherence score
 * - Remaining weight to goal
 *
 * CRITICAL: Handles insufficient data states gracefully
 * CRITICAL: Uses Argentina timezone for all date formatting
 */
export const GoalInsightsCard = () => {
  ...
  // Format goal date in Argentina locale with useMemo
  ...
  if (remainingWeight !== null && remainingWeight <= 0) {
    return (
      <div className="card bg-gradient-to-br from-green-50 to-emerald-50 p-6 shadow-sm rounded-2xl border border-green-100">
        <div className="text-center py-6">
          ...
          <p className="text-sm text-green-700 mt-2">
            Has llegado a tu peso objetivo de 75 kg
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="card bg-white p-6 shadow-sm rounded-2xl border border-gray-100">
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-semibold text-gray-600">Calculando...</h3>
  ...
  const getTrendDisplay = () => {
    if (!currentTrend) {
      return { icon: '—', text: 'Sin datos', color: 'text-gray-400 bg-gray-100 border-gray-200' };
    }
  ...
  return (
    <div className="card bg-white p-6 shadow-sm rounded-2xl border border-gray-100 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-3 gap-3">
          {/* Trend Badge */}
          <div className={`rounded-xl p-3 border ${trendDisplay.color} transition-colors`}>
          ...
          {/* Adherence Badge */}
          <div className={`rounded-xl p-3 border ${adherenceColor} transition-colors`}>
            <div className="text-2xl font-bold mb-0.5">
              {weeklyAdherence}%
            </div>
