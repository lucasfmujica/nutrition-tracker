 ...
 *
 * Shows 4 protein slots as a horizontal timeline with:
 * - Target/consumed progress for each slot
 * - Pulse animation for missed slots
 * - Remaining protein counter
 *
 * Premium Minimalist styling with Inter font, high whitespace.
 */
export const ProteinTimeline = ({ slots, remaining, targetProtein }) => {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900">Protein Pacing</h3>
            <p className="text-[10px] text-gray-500">Distribución óptima de proteína</p>
          </div>
        </div>
          ...
          {slots.map((slot, index) => {
            ...
            return (
              <div key={slot.id} className="flex flex-col items-center">
                {/* Circle Progress */}
                <div className={`relative z-10 flex items-center justify-center w-10 h-10 rounded-full border-2 bg-white ${styles.border} ${styles.ring}`}>
                  <div
                  ...
                  />
                  <span className={`relative text-[10px] font-bold ${slot.progress > 0.5 ? 'text-white' : styles.text}`}>
